...
public:
	...
	void SetCanvasRect()
	{
		const Vec2 cursor_screen_pos = ImGui::GetCursorScreenPos();
		const Vec2 content_region_avail = ImGui::GetContentRegionAvail();
	...
	void SetPlotRect(const float axis_gap_back, const float axis_gap_front)
	{
		...
		plot_rect = rect4f(canvas_rect.LT() + Vec2(axis_gap_back * 2, axis_gap_back), canvas_rect.RB() + Vec2(-axis_gap_back, -axis_gap_back * 2));
	}
	...
	void ProceedInteraction()
	{
		...
		origin = Vec2(canvas_rect.L() + scrolling.x, canvas_rect.T() + scrolling.y);
	}
	...
	void ProceedAxes()
	{
		...
		x_axisline_p0 = Vec2(canvas_rect.L() + axis_gap_back * 2, canvas_rect.B() - axis_gap_back);
		x_axisline_p1 = Vec2(canvas_rect.R() - axis_gap_back, canvas_rect.B() - axis_gap_back);

		y_axisline_p0 = Vec2(canvas_rect.L() + axis_gap_back, canvas_rect.B() - axis_gap_back * 2);
		y_axisline_p1 = Vec2(canvas_rect.L() + axis_gap_back, canvas_rect.T() + axis_gap_back);
	...
	void ProceedGrid()
	{
		...
		Vec2 grid_step = transform_coordinate_system.ScaleToCanvas(Vec2(grid_gaps[0], grid_gaps[1]));
		...
		{
			std::array<Vec2, 2> current_line = { Vec2(x, plot_rect.T()) , Vec2(x, plot_rect.B()) };
			vertical_grid.push_back(current_line);

			std::array<Vec2, 2> current_tick = { Vec2(x, x_axisline_p0.y) , Vec2(x, x_axisline_p0.y + tick_lenght) };
			...
			std::tuple<Vec2, std::string> current_label = std::make_tuple(current_tick[1], stream.str());
		...
		{
			std::array<Vec2, 2> current_line = { Vec2(plot_rect.L(), y), Vec2(plot_rect.R(), y) };
			horizontal_grid.push_back(current_line);

			std::array<Vec2, 2> current_tick = { Vec2(y_axisline_p0.x, y), Vec2(y_axisline_p0.x - tick_lenght, y) };
			x_axis_ticks.push_back(current_tick);
	...
	void SetPlotCurve(const Ty distribution)
	{
		...
		std::vector<Vec2> pdf_curve(steps);

		for (size_t index = 0; index < steps; ++index)
		{
			const float float_index = static_cast<double>(index);
			...
			pdf_curve[index] = Vec2(current_x_value, current_y_value);
		}
	...
	void SetHistogram(const histogram::histogram<std::tuple<histogram::axis::regular<>>>& histogram)
	{
		...
		for (histogram::axis::index_type index = 1; index < histogram.size() - 1; ++index)
		{
			...
			const Vec2 p0 = transform_coordinate_system.TransformToCanvas(Vec2(bin_lower, bin_height));
			const Vec2 p1 = transform_coordinate_system.TransformToCanvas(Vec2(bin_upper, 0.f));
	...
	void Draw()
	{
		...
		for (const auto& label : x_axis_labels)
		{
			...
			Vec2 font_size_vec = font->CalcTextSizeA(font_size, FLT_MAX, 0.0f, text.c_str());
...
private:
	...
	Vec2 cs_origin_x_axis;
	Vec2 cs_origin_y_axis;

	std::vector<std::array<Vec2, 2>> vertical_grid;
	std::vector<std::array<Vec2, 2>> horizontal_grid;

	std::vector<std::array<Vec2, 2>> x_axis_ticks;
	std::vector<std::array<Vec2, 2>> y_axis_ticks;

	std::vector<std::tuple<Vec2, std::string>> x_axis_labels;
	std::vector<std::tuple<Vec2, std::string>> y_axis_labels;

	std::vector<Vec2> transformed_curve;
	std::vector<std::array<Vec2, 2>> bin_array;
